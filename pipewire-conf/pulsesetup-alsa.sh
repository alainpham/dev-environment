...
mics=($(pacmd list-sources | grep -oP 'name: <\K[^>]+'))
...
speakers=($(pacmd list-sinks | grep -oP 'name: <\K[^>]+'))
...
# connect from-desktop to speakers and to-caller-sink
pactl load-module module-combine-sink adjust_time=0 format=s16le rate=48000 channels=2 sink_name=from-desktop slaves=${speaker},to-caller-alsa-sink  sink_properties=device.description="from-desktop"

# connect from-caller to speakers for better control
pactl load-module module-combine-sink adjust_time=0 format=s16le rate=48000 channels=2 sink_name=from-caller slaves=${speaker}  sink_properties=device.description="from-caller"
...
pactl load-module module-loopback adjust_time=0 format=s16le rate=48000  remix=true source="mic01-processed" sink=to-caller-alsa-sink latency_msec=5 source_dont_move=true sink_dont_move=true  >> /tmp/pulsemodule.log
pactl load-module module-loopback adjust_time=0 format=s16le rate=48000  remix=true source="mic02-processed" sink=to-caller-alsa-sink latency_msec=5 source_dont_move=true sink_dont_move=true  >> /tmp/pulsemodule.log
